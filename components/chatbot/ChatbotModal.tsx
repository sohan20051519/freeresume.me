...
const ChatbotModal: React.FC<ChatbotModalProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        const initChat = async () => {
            try {
                ...
                const response = await chatSession.sendMessage({ message: "Start the conversation by greeting me and asking for my full name and job title." });
                setMessages([{ role: 'model', text: response.text }]);
            } catch (err) {
                console.error("Chat initialization error:", err);
                setError("Failed to start the AI assistant. Please check your API key and network connection.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleFinish = async () => {
        if (!chat || isLoading || isFinishing) return;
        ...
        try {
            const response = await chat.sendMessage({ message: finishMessage.text });
            ...
            setError("The AI failed to create the resume data. You can try asking it to 'generate the JSON now', or close this and build your resume manually.");
            setIsLoading(false);
